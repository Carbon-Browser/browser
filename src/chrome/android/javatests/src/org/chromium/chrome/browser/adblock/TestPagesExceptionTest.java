...
public class TestPagesExceptionTest {
    ...
    @LargeTest
    @Feature({"adblock"})
    public void testVerifyElemHideExceptions() throws TimeoutException, InterruptedException {
        mHelper.loadUrl(
                TestPagesTestsHelper.EXCEPTION_DISTRIBUTION_UNIT_TESTCASES_ROOT + "elemhide");
        Assert.assertEquals(1, mHelper.numBlocked());
        Assert.assertEquals(1, mHelper.numBlockedByType(AdblockContentType.CONTENT_TYPE_IMAGE));
        Assert.assertTrue(mHelper.isBlocked(
                TestPagesTestsHelper.DISTRIBUTION_UNIT_RESOURCES_ROOT + "elemhide/image.png"));
        mHelper.verifyHiddenCount(1, "img[data-expectedresult='fail']");
        String numDivs = JavaScriptUtils.executeJavaScriptAndWaitForResult(
                mHelper.getWebContents(), "document.getElementsByTagName(\"div\").length;");
        Assert.assertNotEquals("0", numDivs);
    ...
    @LargeTest
    @Feature({"adblock"})
    public void testVerifyElemHideEmuExceptions() throws TimeoutException, InterruptedException {
        mHelper.loadUrl(
                TestPagesTestsHelper.EXCEPTION_DISTRIBUTION_UNIT_TESTCASES_ROOT + "element-hiding");
        String numDivs = JavaScriptUtils.executeJavaScriptAndWaitForResult(
                mHelper.getWebContents(), "document.getElementsByTagName(\"div\").length;");
        Assert.assertNotEquals("0", numDivs);
        mHelper.verifyDisplayedCount(1, "div[data-expectedresult='pass']");
    ...
    public void testVerifyPopupException() throws TimeoutException, InterruptedException {
        ...
        Assert.assertEquals(3, mHelper.numAllowed());
        Assert.assertEquals(3, mHelper.numAllowedByType(AdblockContentType.CONTENT_TYPE_POPUP));
        Assert.assertTrue(mHelper.isAllowed(TestPagesTestsHelper.DISTRIBUTION_UNIT_RESOURCES_ROOT
    ...
    public void testVerifyDocumentException() throws TimeoutException, InterruptedException {
        ...
        mHelper.numAllowedByType(AdblockContentType.CONTENT_TYPE_DOCUMENT);
