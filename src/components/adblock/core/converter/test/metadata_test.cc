...
class AdblockParserMetadataTest : public testing::Test {
 public:
  absl::optional<adblock::Metadata> ParseHeader(std::string header) {
    ...
    return adblock::Metadata::Parse(input);
  }
};

TEST_F(AdblockParserMetadataTest, EmptyInput) {
  std::stringstream input("");
  auto metadata = adblock::Metadata::Parse(input);
...
TEST_F(AdblockParserMetadataTest, ValidShortestHeader) {
...
TEST_F(AdblockParserMetadataTest, MandatoryAdblockHeaderPresent) {
  ...
  EXPECT_EQ(metadata->GetHomepage(), "");
  EXPECT_FALSE(metadata->GetRedirectUrl().has_value());
  EXPECT_EQ(metadata->GetTitle(), "");
  EXPECT_EQ(metadata->GetVersion(), "");
  EXPECT_EQ(metadata->GetExpires().InDays(),
...
TEST_F(AdblockParserMetadataTest, HomepageIsSet) {
  ...
  ASSERT_TRUE(metadata.has_value());
  EXPECT_EQ(metadata->GetHomepage(), "https://easylist.to/");
}

TEST_F(AdblockParserMetadataTest, RedirectUrlIsSet) {
  auto metadata = ParseHeader("! Redirect: https://redirect-easylist.to/");
  ASSERT_TRUE(metadata.has_value());
  ASSERT_TRUE(metadata->GetRedirectUrl().has_value());
  EXPECT_EQ(metadata->GetRedirectUrl().value().spec(),
...
TEST_F(AdblockParserMetadataTest, ExpirationTimeIsSetHoursExplicit) {
  ...
  ASSERT_TRUE(metadata.has_value());
  EXPECT_EQ(metadata->GetExpires().InHours(), 2);
}
...
TEST_F(AdblockParserMetadataTest, ExpirationTimeIsSetHoursDoubleDigit) {
  ...
  ASSERT_TRUE(metadata.has_value());
  EXPECT_EQ(metadata->GetExpires().InDays(), 2);
}
...
TEST_F(AdblockParserMetadataTest, InvalidExpirationTimeLowerLimit) {
  ...
  ASSERT_TRUE(metadata.has_value());
  EXPECT_EQ(metadata->GetExpires().InHours(),
...
TEST_F(AdblockParserMetadataTest, AllMembersSet) {
  ...
  EXPECT_EQ(metadata->GetTitle(), "EasyList");
  EXPECT_EQ(metadata->GetVersion(), "202204050843");
  EXPECT_EQ(metadata->GetExpires().InMilliseconds(), 2 * 3600000);
}
...
TEST_F(AdblockParserMetadataTest, ParsingStopsAtFirstNotHeaderLine) {
  ...
  auto metadata = Metadata::Parse(input);
  ASSERT_TRUE(metadata.has_value());
  EXPECT_EQ(metadata->GetTitle(), "EasyList");
  EXPECT_EQ(metadata->GetVersion(), "");
