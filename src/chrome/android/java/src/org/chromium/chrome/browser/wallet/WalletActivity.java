...
public class WalletActivity extends ChromeBaseAppCompatActivity implements WalletInterface {
    ...
    private void getTokenTrx() {
        ...
        // Use a single-thread executor to process tasks sequentially but asynchronously
        ExecutorService executor = Executors.newSingleThreadExecutor();

        for (TokenObj token : mTokenArray) {
            executor.execute(() -> {
                try {
                      // Handle non-custom token types
                      switch (token.ticker) {
                          case "ETH":
                              mCryptoAPIHelper.getBSCETHTrx(
                                  mWallet.getAddressForCoin(CoinType.ETHEREUM),
                                  true,
                                  token.ticker,
                                  token.decimals
                              );
                              mCryptoAPIHelper.getTokenNonce(
                                  "ERC20",
                                  mWallet.getAddressForCoin(CoinType.ETHEREUM),
                                  "ETH"
                              );
                          ...
                          case "BSC":
                              mCryptoAPIHelper.getBSCETHTrx(
                                  mWallet.getAddressForCoin(CoinType.SMARTCHAIN),
                                  false,
                                  token.ticker,
                                  token.decimals
                              );
                          ...
                          default:
                              // Handle other token types (e.g., BTC)
                              break;
                        }

                    // Simulate a delay of 2 seconds between tasks
                    Thread.sleep(3500);
    ...
    private void fetchBalance() {
        CryptoAPIHelper mCryptoAPIHelper = new CryptoAPIHelper(this);
        mCryptoAPIHelper.getEthBalance(mWallet.getAddressForCoin(CoinType.ETHEREUM), "18");
        mCryptoAPIHelper.getBNBBalance(mWallet.getAddressForCoin(CoinType.SMARTCHAIN), "18"); // BSC uses eth smart contracts
        ...
        try {
            for (int i = 0; i != mTokenArray.size(); i++) {
                ...
                if (token.isCustomType) {
                    if (token.chainName.equals("BEP20")) {
                        mCryptoAPIHelper.getBEPBalance(token.chain, mWallet.getAddressForCoin(CoinType.SMARTCHAIN), token.ticker, token.decimals);
                    } else {
                        mCryptoAPIHelper.getERCBalance(token.chain, mWallet.getAddressForCoin(CoinType.ETHEREUM), token.ticker, token.decimals);
                    }
                }
            }
        } catch (Exception e) { }
    ...
    public void sendTransaction(TransactionCallback callback) {
        try {
            ...
            // } else {
                ...
                final BigDecimal tokenDecimals = BigDecimal.TEN.pow(Integer.parseInt(mPendingTrx.decimals));
                ...
                amountDecimal = amountDecimal.multiply(tokenDecimals);
                ...
                String nonce =  TokenDatabase.getInstance(this).getTokenNonce(mPendingTrx.chainType.equals("BEP20") ? "BSC" : "ETH");
                ...
                if ((coinType == CoinType.ETHEREUM || coinType == CoinType.SMARTCHAIN) && (mPendingTrx.contractAddress.equals("") || mPendingTrx.contractAddress == null || mPendingTrx.contractAddress.equals("null"))) {
                    ...
                    Ethereum.SigningOutput signerOutput = (Ethereum.SigningOutput)AnySigner.sign(signerInput, coinType, Ethereum.SigningOutput.parser());
    ...
    public void onNavigateSend(TokenObj tokenObj) {
        ...
        walletSendBundle.putString("COIN_DECIMALS", tokenObj.decimals);
    ...
    public void onNavigateVerifyTrx(String mCoinName, int mCoinIcon, String mCoinIconUrl, String mCoinTicker, String mCoinBalance, int mCoinType,
                                    String mRecipientAddress, String mAmount, String mGas, String mCoinUSDValue, String mContractAddress, String mChainType, String mDecimals) {
        ...
        mPendingTrx = new TrxObj(mCoinIcon, mCoinIconUrl, mCoinUSDValue, mAmount, date, "",
                mContractAddress, false, mChainType, mRecipientAddress, 1, mCoinName, mCoinType, mGas, mCoinTicker, mDecimals);
    ...
    private void fetchWallet() {
        ...
        mWallet = new HDWallet(mnemonic, "", false);
    ...
    private void importWallet() {
        try {
            mWallet = new HDWallet(mMnemonicString, "", false);
    ...
    private void createWallet() {
        mWallet = new HDWallet(mWallet.mnemonic(), "", false);
    ...
    @Override
    public void onNavigateViewSeed() {
        getSupportFragmentManager().beginTransaction()
                .addToBackStack(null)
                .replace(R.id.wallet_fragment_container_view, WalletViewSeed.class, null)
                .commit();
    }
