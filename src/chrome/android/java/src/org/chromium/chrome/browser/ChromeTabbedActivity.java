...
public class ChromeTabbedActivity extends ChromeActivity<ChromeActivityComponent>
        implements ChromeAccessibilityUtil.Observer, RewardsAPIBridge.ReviewInterface {
    ...
    public void performPostInflationStartup() {
        ...
        // TODODO
        RewardsAPIBridge.getInstance().setupRewardsInterface(this);
    }
    ...
    public void showRatingPopup() {
        // TODODO
        try {
            ReviewManager manager = /*new FakeReviewManager(this);*/ReviewManagerFactory.create(this);
            ...
            request.addOnCompleteListener(task -> {
                      if (task.isSuccessful()) {
                          ...
                          flow.addOnCompleteListener(flowTask -> {
                              ...
                              // make sure the below is the same as KEY_OPT_OUT in ReviewAPIBridge
                              mEditor.putBoolean("rating_dialog_opt_out", true);
                              mEditor.apply();

                              Amplitude.getInstance().logEvent("rating_dialog_conversion_event");

                              if (!flowTask.isSuccessful()) {
                                  // TODODO show rating dialog instead, and take them to Google Play the old fashioned
                              }
                          });
                      } else {
                          // TODODO show rating dialog instead, and take them to Google Play the old fashioned way
                      }
            });
        } catch (Exception e) { }
    }
