...
class AutocompleteMediator implements OnSuggestionsReceivedListener,
                                      OmniboxSuggestionsDropdown.Observer, SuggestionHost, OmniboxSuggestionsDropdownAdapter.ResultsCommunicator {
    ...
    private OmniboxSuggestionsDropdownAdapter mOmniboxSuggestionsDropdownAdapter;
    ...
    public void setAdapter(OmniboxSuggestionsDropdownAdapter adapter) {
        mOmniboxSuggestionsDropdownAdapter = adapter;
        mOmniboxSuggestionsDropdownAdapter.setCommunicator(this);
    }

    @Override
    public void onNewSuggestionsReceived(ArrayList<VeveSuggestionObj> suggestions) {

        // mAutocompleteResult.updateMatchesNotNative(suggestions);
        List<AutocompleteMatch> currSuggestions = mAutocompleteResult.getSuggestionsList();
        boolean didReplace = false;
        for (int i = 0; i != currSuggestions.size(); i++) {
            for (int z = 0; z != suggestions.size(); z++) {
                if (currSuggestions.get(i).mDisplayText.equals(suggestions.get(z).durl)) {
                    currSuggestions.get(i).updateUrl(suggestions.get(z).url);
                    didReplace = true;
                }
            }
        }
        ...
        mAutocompleteResult.updateSponsoredSuggestions(currSuggestions);


        mAutocomplete.onUpdatedSuggestionsReceived(mAutocompleteResult);
        //mOmniboxSuggestionsDropdownAdapter.notifyDataSetChanged();
    }
    ...
    public void onSuggestionsReceived(
        ...
        if (!mAutocompleteResult.equals(autocompleteResult)) {
            ...
            String keyword = null;
            final String urlBarText = mUrlBarEditingTextProvider.getTextWithoutAutocomplete();
            if (!mDataProvider.getTab().isIncognito() && !urlBarText.startsWith("http")) {
                if (newSuggestions != null && !newSuggestions.isEmpty()) {
                    try {
                        String firstSuggestion = newSuggestions.get(0).getDisplayText();
                        String suggestionText = urlBarText.equals(firstSuggestion) ? newSuggestions.get(1).getDisplayText()
                                                                            : firstSuggestion;

                        keyword = suggestionText.substring(0, suggestionText.indexOf(" "));
                    } catch (Exception e) {

                        if (!urlBarText.contains(" "))
                            keyword = urlBarText;
                    }
                }
            }

            // After the user has entered at least 4 characters in the url bar, send to filter for sponsored suggestions from SitePlug
            if (keyword != null && keyword.length() > 3 && mOmniboxSuggestionsDropdownAdapter != null)
                mOmniboxSuggestionsDropdownAdapter.getFilter().filter(keyword);
    ...
    private void loadUrlForOmniboxMatch(int matchPosition, @NonNull AutocompleteMatch suggestion,
        ...
        if (mRewardsBridge == null) mRewardsBridge = RewardsAPIBridge.getInstance();
        mRewardsBridge.logSearch();
