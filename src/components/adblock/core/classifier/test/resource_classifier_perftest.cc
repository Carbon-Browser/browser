...
class ResourceClassifierPerfTest : public testing::Test {
 public:
  ...
  void MeasureUrlMatchingTime(
    ...
    LOG(INFO) << "URL matching time: "
              << timer.Elapsed().InMicroseconds() / cycles << " us";
  ...
  void MeasureCSPMatchingTime(
    ...
    LOG(INFO) << "CSP filter search time: "
              << timer.Elapsed().InMicroseconds() / cycles << " us";
  ...
  void MeasureElemhideGenerationTime(
    ...
    LOG(INFO) << "Elemhide generation time: "
              << timer.Elapsed().InMicroseconds() / BenchmarkRepetitions()
              << " us";
  }
...
TEST_F(ResourceClassifierPerfTest, NoWildcardDomainElemhideMatch) {
...
TEST_F(ResourceClassifierPerfTest, NoWildcardMultipleDomainsElemhideMatch) {
...
TEST_F(ResourceClassifierPerfTest, WildcardDomainElemhideMatch) {
  auto state =
...
TEST_F(ResourceClassifierPerfTest, NoWildcardDomainElemhideNoMatch) {
  auto state = CreateStateFromFilters(
      std::vector<std::string>{"example.com###selector"});
...
TEST_F(ResourceClassifierPerfTest, NoWildcardMultipleDomainsElemhideNoMatch) {
  auto state = CreateStateFromFilters(std::vector<std::string>{
      "example.de,example.pl,example.net,example.co.uk,example.com,example.fr, "
      "example.hu###selector"});
  MeasureElemhideGenerationTime(GURL("https://wrong.com/frame.html"),
...
TEST_F(ResourceClassifierPerfTest, WildcardDomainElemhideNoMatch) {
  auto state =
      CreateStateFromFilters(std::vector<std::string>{"example.*###selector"});
  MeasureElemhideGenerationTime(GURL("https://wrong.com/frame.html"),
                                std::move(state));
...
TEST_F(ResourceClassifierPerfTest, NoWildcardDomainUrlMatch) {
  auto state = CreateStateFromFilters(
...
TEST_F(ResourceClassifierPerfTest, NoWildcardMultipleDomainsUrlMatch) {
  auto state = CreateStateFromFilters(
...
TEST_F(ResourceClassifierPerfTest, WildcardDomainUrlMatch) {
  auto state =
...
TEST_F(ResourceClassifierPerfTest, NoWildcardDomainUrlNoMatch) {
  auto state = CreateStateFromFilters(
...
TEST_F(ResourceClassifierPerfTest, NoWildcardMultipleDomainsUrlNoMatch) {
  auto state = CreateStateFromFilters(
...
TEST_F(ResourceClassifierPerfTest, WildcardDomainUrlNoMatch) {
  auto state =
