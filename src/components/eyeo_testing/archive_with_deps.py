...
def get_executable():
    third_party_instance = os.path.join(repo_root, "third_party", "lzma_sdk", "bin",
                                        "host_platform", "7za")
...
def rebase_paths(input_paths, build_dir):
    ...
    absolute_paths = [os.path.abspath(os.path.join(
        build_dir, path)) for path in input_paths]
...
def main():
    ...
    compress_paths(inputs_relative_to_root, os.path.abspath(
        os.path.join(args.build_dir, args.output)))
