...
#ifndef COMPONENTS_ADBLOCK_ADBLOCK_CONTROLLER_H_
#define COMPONENTS_ADBLOCK_ADBLOCK_CONTROLLER_H_
...
class AdblockController : public KeyedService {
 public:
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnRecommendedSubscriptionsAvailable(
  ...
  virtual std::vector<GURL> GetSelectedBuiltInSubscriptions() const = 0;

  virtual void AddCustomSubscription(const GURL& url) = 0;
  virtual void RemoveCustomSubscription(const GURL& url) = 0;
  virtual std::vector<GURL> GetCustomSubscriptions() const = 0;
  ...
  virtual std::vector<Subscription> GetRecommendedSubscriptions() const = 0;
};
